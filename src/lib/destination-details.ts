...
export type DestinationDetail = {
  ...
  relatedDestinations: DestinationRelated[];
};
...
const destinationsData: Record<string, DestinationDetail> = {
  portugal: {
    ...
    ilRankShort: "#1",
    tagline:
      "Europe's #1 retirement destination · 300+ sunny days · EU citizenship after 5 years",
    budgetShort: "$2,500",
    bestCity: "Algarve",
    taxRate: "10%",
    qolScore: "9/10",
    ...
    healthcare: {
      description:
        "Portugal's SNS (Serviço Nacional de Saúde) is ranked among Europe's best and is free for all legal residents. Private insurance costs $100–$200/month and gives fast access to English-speaking specialists.",
      keyTakeaway:
        "EU-standard public healthcare free for residents plus excellent affordable private care. Strong English-speaking medical access throughout the Algarve.",
    },
    visa: {
      name: "D7 Passive Income Visa",
      requirements: "Minimum income: ~$1,400/month · Social Security accepted ✓",
      description:
        "The D7 requires ~$1,400/month in passive income, including pensions, Social Security, and investments. It leads to a 2-year permit, renewable for 3-year periods. After 5 years you qualify for permanent residency and EU citizenship.",
      keyTakeaway:
        "One of Europe's most accessible retirement visas. Social Security counts as qualifying income and the 5-year EU citizenship path is unmatched globally.",
    },
    tax: {
      description:
        "NHR 2.0 offers new residents a 10% flat tax on foreign pension income for 10 years. The US-Portugal tax treaty and totalization agreement help prevent double taxation on Social Security income.",
      keyTakeaway:
        "NHR 2.0 is one of Europe's best retiree tax regimes. A 10% flat rate plus a US treaty makes Portugal highly tax-efficient compared to France or Spain.",
    },
    scores: {
      healthcare: 95,
      safety: 90,
...
function readDestinationsJson(): Record<string, DestinationDetail> | null {
...
export async function getAllDestinationSlugs(): Promise<string[]> {
  const jsonData = readDestinationsJson();
  return Object.keys(jsonData || destinationsData);
}
...
export async function getDestinationBySlug(
  ...
  const jsonData = readDestinationsJson();
  const data = jsonData || destinationsData;
