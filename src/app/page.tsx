...
export default function RetirementFinder() {
  ...
  const [answers, setAnswers] = useState({ who:"", budget:"", income:"", region:"", lifestyle:"", english:false, taxFree:false, easyVisa:false });
  const [results, setResults] = useState(null);

  function set(key, val) { setAnswers(a => ({...a, [key]: val})); }
  function toggle(key) { setAnswers(a => ({...a, [key]: !a[key]})); }
  ...
  function findMatches() {
    ...
    // HARD filters first — region and income type eliminate non-matching countries entirely
  ...
  if (results) {
    return (
      <div style={{background:"#fff",borderRadius:16,padding:"32px 28px",maxWidth:640,margin:"0 auto",boxShadow:"0 4px 32px rgba(0,0,0,0.10)"}}>
        <div style={{textAlign:"center",marginBottom:24}}>
          <div style={{fontSize:32,marginBottom:8}}>🎯</div>
          <div style={{fontSize:20,fontWeight:700,color:"#111",marginBottom:4}}>Your top retirement matches</div>
          <div style={{fontSize:13,color:"#888"}}>Based on your answers — click any card to explore</div>
        </div>
        {results.length === 0 ? (
          <div style={{textAlign:"center",padding:"32px 0",color:"#888"}}>
            <div style={{fontSize:32,marginBottom:12}}>🔍</div>
            <div style={{fontSize:15,fontWeight:600,marginBottom:8}}>No exact matches found</div>
            <div style={{fontSize:13}}>Try selecting "Open to anywhere" or a different income type.</div>
          </div>
        ) : (
          <div style={{display:"flex",flexDirection:"column",gap:14,marginBottom:24}}>
            {results.map((c, i) => (
              <a key={c.id} href={`https://golden-horizons.org/destinations/${c.id}`} target="_blank" rel="noreferrer"
                style={{display:"flex",gap:0,background:"#f9f9f9",borderRadius:12,overflow:"hidden",border:"1px solid #eee",textDecoration:"none"}}>
        ...
        <div style={{display:"flex",gap:10}}>
          ...
          <a href="https://golden-horizons.org/destinations" style={{flex:1,padding:"11px",background:dark,color:"#fff",borderRadius:8,fontSize:13,fontWeight:600,textAlign:"center",textDecoration:"none",display:"flex",alignItems:"center",justifyContent:"center"}}>See All 26 Countries →</a>
        </div>
      </div>
    );
  }
  ...
  return (
    <div style={{background:"#fff",borderRadius:16,padding:"32px 28px",maxWidth:560,margin:"0 auto",boxShadow:"0 4px 32px rgba(0,0,0,0.10)"}}>
      {/* Progress */}
      ...
      <div style={{fontSize:11,color:"#bbb",textAlign:"right",marginBottom:22}}>Step {step+1} of {STEPS.length}</div>
      ...
      {step === 0 && (
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
          {[["single","Solo retiree","👤"],["couple","Retiring as a couple","👫"]].map(([v,label,icon]) => (
            <button key={v} onClick={() => set("who", v)} style={{padding:"22px 12px",borderRadius:12,border:`2px solid ${answers.who===v?gold:"#e5e5e5"}`,background:answers.who===v?"#fffbf0":"#fff",cursor:"pointer",textAlign:"center",transition:"all 0.15s"}}>
      ...
      {step === 1 && (
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
          {[["under1500","Under $1,500","🟢 Very budget"],["1500_2500","$1,500–$2,500","🟡 Comfortable"],["2500_4000","$2,500–$4,000","🟠 Premium"],["4000plus","$4,000+","🔵 Luxury"]].map(([v,label,sub]) => (
            <button key={v} onClick={() => set("budget", v)} style={{padding:"18px 12px",borderRadius:12,border:`2px solid ${answers.budget===v?gold:"#e5e5e5"}`,background:answers.budget===v?"#fffbf0":"#fff",cursor:"pointer",textAlign:"center",transition:"all 0.15s"}}>
      ...
      {step === 2 && (
        <div style={{display:"flex",flexDirection:"column",gap:12}}>
          {[
            ["ss","🏛️ Social Security","Monthly SS check from the US government — accepted by most retirement visas"],
            ["savings","🏦 Savings / Investments","Bank statements or investment portfolio showing sufficient funds"],
            ["other","💼 Pension / Rental / Business","Private pension, rental income, annuity, or business income from abroad"]
          ].map(([v, label, desc]) => (
            <button key={v} onClick={() => set("income", v)} style={{display:"flex",alignItems:"flex-start",gap:14,padding:"16px",borderRadius:12,border:`2px solid ${answers.income===v?gold:"#e5e5e5"}`,background:answers.income===v?"#fffbf0":"#fff",cursor:"pointer",textAlign:"left",transition:"all 0.15s"}}>
      ...
      {step === 3 && (
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
          {[["europe","🇪🇺 Europe"],["americas","🌎 The Americas"],["asia","🌏 Asia"],["any","🌍 Open to anywhere"]].map(([v, label]) => (
            <button key={v} onClick={() => set("region", v)} style={{padding:"20px 12px",borderRadius:12,border:`2px solid ${answers.region===v?gold:"#e5e5e5"}`,background:answers.region===v?"#fffbf0":"#fff",cursor:"pointer",textAlign:"center",fontSize:14,fontWeight:600,color:"#222",transition:"all 0.15s"}}>
              {label}
            </button>
          ))}
        </div>
      ...
      {step === 4 && (
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
          {[["beach","🏖️ Beach & coast","Sun, sand, diving"],["city","🏙️ City life","Culture, dining, walkable"],["countryside","🌿 Countryside","Nature, quiet, mountains"],["any","✨ Surprise me","Best overall match"]].map(([v,icon,sub]) => (
            <button key={v} onClick={() => set("lifestyle", v)} style={{padding:"18px 12px",borderRadius:12,border:`2px solid ${answers.lifestyle===v?gold:"#e5e5e5"}`,background:answers.lifestyle===v?"#fffbf0":"#fff",cursor:"pointer",textAlign:"center",transition:"all 0.15s"}}>
      ...
      {step === 5 && (
        <div style={{display:"flex",flexDirection:"column",gap:10}}>
          <div style={{fontSize:12,color:"#999",textAlign:"center",marginBottom:4}}>Select any that matter — or skip all and find matches</div>
          {[
            ["english","🗣️ English widely spoken","No language barrier in daily life"],
            ["taxFree","💵 Tax-free foreign income","Keep more of your SS & pension"],
            ["easyVisa","✅ Easy visa / residency","Straightforward, fast approval"]
          ].map(([key, label, sub]) => (
      ...
      <div style={{display:"flex",gap:10,marginTop:24}}>
        ...
        <button onClick={next} disabled={!canNext}
          style={{flex:1,padding:"13px",background:canNext?dark:"#ccc",color:"#fff",border:"none",borderRadius:8,fontSize:14,fontWeight:700,cursor:canNext?"pointer":"not-allowed",transition:"background 0.2s"}}>
