...
const DESTINATIONS: Destination[] = [
  ...
  { id: "spain", name: "Spain", flag: "🇪🇸", rank: 4, lat: 40.4, lng: -3.7, cost: "$2,500–$4,500/mo", region: "Europe", tagline: "Mediterranean lifestyle", articles: [{ cat: "Beach", title: "Mediterranean Oceanfront Living in Málaga Province", slug: "spain-mediterranean-oceanfront-living-in-malaga-province" }, { cat: "Beach", title: "Beach Retirement Living on Costa del Sol", slug: "spain-beach-retirement-living-on-costa-del-sol" }] },
  { id: "panama", name: "Panama", flag: "🇵🇦", rank: 5, lat: 8.9, lng: -79.5, cost: "$2,500–$4,500/mo", region: "Central America", tagline: "Uses US dollar, Pensionado discounts", articles: [{ cat: "Safety", title: "Safest Areas for Retirees in Panama", slug: "panama-safest-areas-for-retirees-in-panama" }, { cat: "Expat", title: "Expat Life in Boquete's Coffee Highlands", slug: "panama-expat-life-in-boquetes-coffee-highlands" }] },
  ...
  { id: "vietnam", name: "Vietnam", flag: "🇻🇳", rank: null, lat: 14.0, lng: 108.3, cost: "$1,000–$1,800/mo", region: "SE Asia", tagline: "Southeast Asia's best value", articles: [{ cat: "Best Cities", title: "Best Cities for Retirees: Da Nang vs Hội An", slug: "vietnam-best-cities-for-retirees-da-nang-vs-hoi-an" }, { cat: "Hidden Gems", title: "Hidden Riverside Towns Beyond the Tourist Trail", slug: "vietnam-hidden-riverside-towns-beyond-the-tourist-trail" }] },
  { id: "italy", name: "Italy", flag: "🇮🇹", rank: null, lat: 42.5, lng: 12.5, cost: "$2,500–$5,000/mo", region: "Europe", tagline: "€100k flat tax & world's best food", articles: [{ cat: "Cost", title: "What $3,000 a Month Gets You in Sicily", slug: "italy-what-3000-a-month-gets-you-in-sicily" }] },
  { id: "france", name: "France", flag: "🇫🇷", rank: null, lat: 46.2, lng: 2.2, cost: "$3,000–$6,000/mo", region: "Europe", tagline: "World's #1 healthcare system", articles: [] },
  ...
  { id: "cambodia", name: "Cambodia", flag: "🇰🇭", rank: null, lat: 12.6, lng: 104.9, cost: "$1,000–$1,800/mo", region: "SE Asia", tagline: "Easiest visa in Asia, USD economy", articles: [] },
  ...
  { id: "philippines", name: "Philippines", flag: "🇵🇭", rank: null, lat: 12.9, lng: 121.8, cost: "$1,200–$2,500/mo", region: "SE Asia", tagline: "English-speaking Asia, no tax on pensions", articles: [{ cat: "Safety", title: "Safest Areas for Retirement in the Philippines", slug: "philippines-safest-areas-for-retirement-in-the-philippines" }] },
  { id: "panama-azores", name: "Azores", flag: "🇵🇹", rank: null, lat: 37.7, lng: -25.5, cost: "$1,800–$2,800/mo", region: "Atlantic Islands", tagline: "Europe's best-kept secret", articles: [] },
];
  ...
  useEffect(() => {
    ...
    script.onload = () => {
      ...
      DESTINATIONS.forEach((dest) => {
        ...
        const icon = L.divIcon({ html: iconHtml, className: "", iconSize: [size, size], iconAnchor: [half, half] });

        const marker = L.marker([dest.lat, dest.lng], { icon }).addTo(map).on("click", () => {
          setSelected(dest);
  ...
  return (
    <div style={{ fontFamily: "'Source Sans 3', sans-serif" }}>
      <div style={{ position: "relative" }}>
        <div ref={mapRef} style={{ width: "100%", height: "480px", borderRadius: "8px", overflow: "hidden", background: "#e8f0f7" }} />
        {!mapReady && (
          <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center", background: "#f0f4f0", borderRadius: "8px" }}>
      ...
      {selected && (
        <div style={{ marginTop: "16px", background: "#ffffff", border: "1px solid #e8e0d0", borderLeft: "4px solid #c8a84e", borderRadius: "8px", padding: "20px 20px 16px" }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
              ...
              <div>
                <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                  ...
                  {selected.rank && (
                    <span style={{ fontSize: "11px", fontWeight: 700, background: "#1a3a2a", color: "#c8a84e", padding: "2px 8px", borderRadius: "4px" }}>#{selected.rank} Ranked</span>
                  )}
                </div>
                <p style={{ fontSize: "13px", color: "#888", margin: "2px 0 0" }}>{selected.region} · {selected.cost} couple</p>
              </div>
            </div>
            <button onClick={() => setSelected(null)} style={{ background: "none", border: "none", cursor: "pointer", fontSize: "20px", color: "#aaa", lineHeight: 1 }}>×</button>
          </div>

          <p style={{ fontSize: "14px", color: "#555", margin: "0 0 14px", fontStyle: "italic" }}>{selected.tagline}</p>

          <Link href={`/destinations/${selected.id}`} style={{ display: "inline-block", background: "#1a3a2a", color: "#ffffff", fontSize: "11px", fontWeight: 700, letterSpacing: "1.5px", textTransform: "uppercase", textDecoration: "none", padding: "10px 18px", borderRadius: "6px", marginBottom: selected.articles.length > 0 ? "14px" : "0" }}>
            View Full Profile →
          </Link>

          {selected.articles.length > 0 && (
            <div style={{ borderTop: "1px solid #f0ece4", paddingTop: "12px" }}>
              <p style={{ fontSize: "10px", fontWeight: 700, letterSpacing: "2px", textTransform: "uppercase", color: "#aaa", margin: "0 0 8px" }}>Related Articles</p>
              <div style={{ display: "flex", flexDirection: "column", gap: "6px" }}>
                {selected.articles.map((a) => (
                  <Link key={a.slug} href={`/articles/${a.slug}`} style={{ display: "flex", alignItems: "center", gap: "8px", textDecoration: "none" }}>
                    <span style={{ fontSize: "10px", fontWeight: 700, background: "#e8f4ed", color: "#0f6e56", padding: "2px 7px", borderRadius: "3px", flexShrink: 0 }}>{a.cat}</span>
